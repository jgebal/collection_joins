...
declare
   v_dummy integer;
begin
   dbms_profiler.START_PROFILER('nl_full_scan_join 1,000 parents 20,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.nl_full_scan_join(1000, 20000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('nl_index_scan_join 1,000 parents 20,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.nl_index_scan_join(1000, 20000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('sort_merge_join 1,000 parents 20,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.sort_merge_join(1000, 20000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('nl_index_scan_join 40,000 parents 400,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.nl_index_scan_join(40000, 400000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('sort_merge_join 40,000 parents 400,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.sort_merge_join(40000, 400000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('nl_index_scan_join 10,000 parents 1,000,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.nl_index_scan_join(10000, 1000000) );
   dbms_profiler.STOP_PROFILER();

   dbms_profiler.START_PROFILER('sort_merge_join 10,000 parents 1,000,000 children');
   select count(1) into v_dummy
   from table( collection_outer_join.sort_merge_join(10000, 1000000) );
   dbms_profiler.STOP_PROFILER();
end;
/

select run_comment, run_total_time/1000000 as total_time_mili_sec from plsql_profiler_runs order by runid;
